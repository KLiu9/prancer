...
@app.route("/api/get_filenames", methods=["POST"])
def get_filenames():
    ...
    if dir == None:
        ...
        filenames = get_filenames_from_directory(dir, ext)
...
@app.route("/api/get_json_file", methods=["POST"])  # this needs to be POST
def get_json_file():
    incoming = request.get_json()
    id = incoming["id"]
    textDir = incoming["dir"]
    file = get_json_file_data(id, textDir)
    if file:
        return jsonify(file=file)
    else:
        return jsonify(error=True), 403
...
def save_annotations():
    ...
    dir = incoming["dir"]
    if dir == None:
        is_saved = save_annotations_file(incoming["id"] + ".json",
                                         incoming["annotations"])
    else:
        is_saved = save_annotations_file(incoming["id"] + ".json",
                                         incoming["annotations"], dir)
    if is_saved:
